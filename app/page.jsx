...
const NAV = [
  ['Today', '⌂'], ['Family Space', '♧'], ['Calendar', '□'], ['Notifications', '◉'], ['My Vault', '⌑'], ['People', '◌'], ['Timeline', '↗'], ['Library', '▧'], ['Agent Studio', '✧']
]
const PEOPLE = [
...
function Library({ memories, profiles, profileFilter, setProfileFilter, query, onCapture, onOpen, loading }) { const selectedProfile = profiles.find(profile => profile.id === profileFilter); return <><PageHeading eyebrow="MEMORIES · FAMILY COLLECTION" title="Your family story." text={selectedProfile ? `Permitted memories linked to ${selectedProfile.display_name}.` : query?`Showing results for “${query}”.`:'Big milestones. Small moments. Everything worth keeping.'} action="＋ Add memory" onAction={onCapture}/><CollectionToolbar profiles={profiles} profileFilter={profileFilter} setProfileFilter={setProfileFilter}/>{loading?<LoadingContent/>:memories.length?<section className="memory-grid-new">{memories.map(memory=><MemoryCard key={memory.id} memory={memory} onOpen={onOpen}/>)}</section>:<EmptyCollection title="No memories found" text="Only items you can access are shown for this profile." onCapture={onCapture}/>}</> }
