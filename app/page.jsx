...
function FamilyApp() {
  const supabase = getSupabaseBrowserClient()
  const [page, setPage] = useState('Today'), [query, setQuery] = useState(''), [memories, setMemories] = useState([]), [spaces, setSpaces] = useState([]), [context, setContext] = useState(null), [notice, setNotice] = useState(''), [captureOpen, setCaptureOpen] = useState(false), [loading, setLoading] = useState(true)
  ...
  return <div className="app-shell"><Sidebar page={page} setPage={setPage} /><main className="main"><header className="topbar"><span className="preview-dot">PRIVATE FAMILY OS</span><label className="search"><span>⌕</span><input aria-label="Search memories" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search memories" /></label></header><div className="content">{notice && <div className="notice" role="status">{notice}</div>}{page === 'Today' && <Home setPage={setPage} memories={memories} onCapture={openCapture} loading={loading} />}{page === 'Family Space' && <FamilyChat supabase={supabase} context={context} spaces={spaces} flash={flash} />}{page === 'Calendar' && <FamilyCalendar supabase={supabase} context={context} spaces={spaces} flash={flash} />}{page === 'My Vault' && <Vault memories={filtered.filter(memory => memory.scope === 'Private')} onCapture={openCapture} loading={loading} />}{page === 'People' && <People supabase={supabase} context={context} spaces={spaces} flash={flash} />}{page === 'Timeline' && <Timeline memories={filtered} onCapture={openCapture} loading={loading} />}{page === 'Library' && <Library memories={filtered} query={query} onCapture={openCapture} loading={loading} />}{page === 'Agent Studio' && <AgentStudio supabase={supabase} context={context} spaces={spaces} flash={flash} />}</div></main>{captureOpen && <CaptureModal supabase={supabase} context={context} spaces={spaces} close={() => setCaptureOpen(false)} onSaved={saved} />}{<nav className="mobile-nav" aria-label="Primary navigation">{NAV.map(([name, icon]) => <button key={name} className={page === name ? 'active' : ''} onClick={() => setPage(name)}><span>{icon}</span>{name}</button>)}</nav>}</div>
}
