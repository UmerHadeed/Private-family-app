...
import CuratedMemories from './curated-memories'
import Privacy from './privacy'
import './page.css'

const NAV = [
  ['Today', '⌂'], ['Family Space', '♧'], ['Calendar', '□'], ['Notifications', '◉'], ['My Vault', '⌑'], ['People', '◌'], ['Timeline', '↗'], ['Library', '▧'], ['Privacy', '♙'], ['Agent Studio', '✧']
]
const PEOPLE = [
...
function AuthGate({ children }) {
  ...
  const [mode, setMode] = useState('signin'), [email, setEmail] = useState(''), [password, setPassword] = useState(''), [message, setMessage] = useState(''), [busy, setBusy] = useState(false), [resetMode, setResetMode] = useState(false)
  useEffect(() => {
    if (!supabase) return
  ...
  const resetPassword = async event => {
    event.preventDefault(); if (!supabase || busy || !email.trim()) return
    setBusy(true); setMessage('')
    ...
    setBusy(false)
    setMessage(error ? error.message : 'If this email has an account, a secure password-reset link has been sent.')
  }
  ...
  if (!state.user && resetMode) return <AuthShell><p className="kicker">ACCOUNT RECOVERY</p><h1>Reset your password.</h1><p>Enter your account email. We will send a reset link if an account exists, without confirming whether that email is registered.</p><form className="auth-form" onSubmit={resetPassword}><label>Email<input type="email" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} required /></label><button className="primary" disabled={busy}>{busy ? 'Sending…' : 'Send reset link'}</button></form>{message && <p className="auth-message" role="status">{message}</p>}<button className="text-button" disabled={busy} onClick={() => { setResetMode(false); setMessage('') }}>Back to sign in</button></AuthShell>
  if (!state.user) return <AuthShell><p className="kicker">PRIVATE FAMILY OS</p><h1>{mode === 'signin' ? 'Welcome back.' : 'Create your private account.'}</h1><p>Sign in to access only the household spaces explicitly shared with you.</p><form className="auth-form" onSubmit={submit}><label>Email<input type="email" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} required /></label><label>Password<input type="password" autoComplete={mode === 'signin' ? 'current-password' : 'new-password'} minLength="8" value={password} onChange={e => setPassword(e.target.value)} required /></label><button className="primary" disabled={busy}>{busy ? 'Please wait…' : mode === 'signin' ? 'Sign in' : 'Create account'}</button></form>{message && <p className="auth-message" role="status">{message}</p>}<div className="auth-divider">or</div><button className="secondary full" disabled={busy} onClick={signInGoogle}>Continue with Google</button>{mode === 'signin' && <button className="text-button" disabled={busy} onClick={() => { setResetMode(true); setMessage('') }}>Forgot password?</button>}<button className="text-button" disabled={busy} onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); setMessage('') }}>{mode === 'signin' ? 'New here? Create an account' : 'Already have an account? Sign in'}</button></AuthShell>
...
function FamilyApp() {
  const supabase = getSupabaseBrowserClient()
  const [page, setPage] = useState('Today'), [query, setQuery] = useState(''), [memories, setMemories] = useState([]), [profiles, setProfiles] = useState([]), [spaces, setSpaces] = useState([]), [context, setContext] = useState(null), [notice, setNotice] = useState(''), [captureOpen, setCaptureOpen] = useState(false), [selectedAsset, setSelectedAsset] = useState(null), [profileFilter, setProfileFilter] = useState(''), [loading, setLoading] = useState(true)
  ...
  const loadContent = useCallback(async () => {
    ...
    const [{ data: profileRows, error: profilesError }, { data: tagRows, error: tagsError }] = await Promise.all([
      supabase.from('family_profiles').select('id,display_name,profile_type').eq('household_id', memberships[0].household_id).order('display_name'),
      assets?.length ? supabase.from('asset_people').select('asset_id,profile_id').in('asset_id', assets.map(asset => asset.id)) : { data: [], error: null }
    ])
    if (profilesError || tagsError) { setLoading(false); return setNotice(profilesError?.message || tagsError?.message || 'We could not load family profile tags.') }
    const spaceById = new Map((accessibleSpaces || []).map(space => [space.id, space]))
    const profilesById = new Map((profileRows || []).map(profile => [profile.id, profile]))
    const tagsByAsset = new Map()
    ;(tagRows || []).forEach(tag => tagsByAsset.set(tag.asset_id, [...(tagsByAsset.get(tag.asset_id) || []), { id: tag.profile_id, name: profilesById.get(tag.profile_id)?.display_name || 'Family profile' }]))
    setContext({ userId: user.id, householdId: memberships[0].household_id })
    setSpaces(accessibleSpaces || []); setProfiles(profileRows || [])
    setMemories((assets || []).map(asset => {
      const space = spaceById.get(asset.space_id)
      return { id: asset.id, title: asset.filename, type: asset.asset_type === 'audio' ? 'Voice note' : asset.asset_type[0].toUpperCase() + asset.asset_type.slice(1), person: space?.name || 'Private space', date: new Intl.DateTimeFormat(undefined, { month: 'short', day: 'numeric', year: 'numeric' }).format(new Date(asset.created_at)), scope: space?.space_type === 'private_vault' ? 'Private' : 'Family', caption: `${asset.mime_type} · ${Math.max(1, Math.ceil(asset.byte_size / 1024))} KB`, voice: asset.asset_type === 'audio', spaceType: space?.space_type, spaceName: space?.name, uploadedBy: asset.uploaded_by_user_id, storagePath: asset.storage_path, mimeType: asset.mime_type, byteSize: asset.byte_size, profileTags: tagsByAsset.get(asset.id) || [] }
    }))
    setLoading(false)
  }, [supabase])
  useEffect(() => { const timer = window.setTimeout(() => { void loadContent() }, 0); return () => window.clearTimeout(timer) }, [loadContent])
  const filtered = useMemo(() => memories.filter(memory => Object.values(memory).join(' ').toLowerCase().includes(query.toLowerCase()) && (!profileFilter || memory.profileTags.some(tag => tag.id === profileFilter))), [memories, query, profileFilter])
  ...
  const openProfileMemories = profile => { setProfileFilter(profile.id); setPage('Library') }
  return <div className="app-shell"><Sidebar page={page} setPage={setPage} /><main className="main"><header className="topbar"><span className="preview-dot">PRIVATE FAMILY OS</span><label className="search"><span>⌕</span><input aria-label="Search memories" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search memories" /></label></header><div className="content">{notice && <div className="notice" role="status">{notice}</div>}{page === 'Today' && <Home setPage={setPage} memories={memories} onCapture={openCapture} onOpen={setSelectedAsset} loading={loading} />}{page === 'Family Space' && <FamilyChat supabase={supabase} context={context} spaces={spaces} flash={flash} />}{page === 'Calendar' && <FamilyCalendar supabase={supabase} context={context} spaces={spaces} flash={flash} />}{page === 'Notifications' && <Notifications supabase={supabase} context={context} flash={flash} />}{page === 'My Vault' && <Vault memories={filtered.filter(memory => memory.scope === 'Private')} profiles={profiles} profileFilter={profileFilter} setProfileFilter={setProfileFilter} onCapture={openCapture} onOpen={setSelectedAsset} loading={loading} />}{page === 'People' && <People supabase={supabase} context={context} spaces={spaces} flash={flash} onViewProfileMemories={openProfileMemories} />}{page === 'Timeline' && <Timeline memories={filtered} profiles={profiles} profileFilter={profileFilter} setProfileFilter={setProfileFilter} onCapture={openCapture} onOpen={setSelectedAsset} loading={loading} />}{page === 'Library' && <Library memories={filtered} allAssets={memories} profiles={profiles} profileFilter={profileFilter} setProfileFilter={setProfileFilter} query={query} onCapture={openCapture} onOpen={setSelectedAsset} loading={loading} supabase={supabase} context={context} spaces={spaces} flash={flash} />}{page === 'Privacy' && <Privacy supabase={supabase} context={context} assets={memories} flash={flash} />}{page === 'Agent Studio' && <AgentStudio supabase={supabase} context={context} spaces={spaces} flash={flash} />}</div></main>{captureOpen && <CaptureModal supabase={supabase} context={context} spaces={spaces} close={() => setCaptureOpen(false)} onSaved={saved} />}{selectedAsset && <AssetDetail asset={selectedAsset} supabase={supabase} context={context} spaces={spaces} profiles={profiles} close={() => setSelectedAsset(null)} onChanged={loadContent} flash={flash} />}{<nav className="mobile-nav" aria-label="Primary navigation">{NAV.map(([name, icon]) => <button key={name} className={page === name ? 'active' : ''} onClick={() => setPage(name)}><span>{icon}</span>{name}</button>)}</nav>}</div>
}
...
function Timeline({ memories, profiles, profileFilter, setProfileFilter, onCapture, onOpen, loading }) { return <><PageHeading eyebrow="YOUR FAMILY STORY" title="Timeline" text="Big milestones. Small moments. Everything worth keeping." action="＋ Add memory" onAction={onCapture}/><CollectionToolbar profiles={profiles} profileFilter={profileFilter} setProfileFilter={setProfileFilter}/>{loading?<LoadingContent/>:<section className="timeline-new">{memories.map(memory=><article key={memory.id} onClick={() => onOpen(memory)}><time>{memory.date}</time><i/><div><p className="kicker">{memory.type} · {memory.scope}</p><h2>{memory.title}</h2><p>{memory.caption}</p><ProfileTagList tags={memory.profileTags}/></div></article>)}</section>}</> }
function CollectionToolbar({ profiles = [], profileFilter = '', setProfileFilter = () => {} }){return <div className="collection-toolbar"><div><button className="active">Browse</button><button>Timeline</button></div><label className="profile-filter">Filter by person<select aria-label="Filter by person" value={profileFilter} onChange={event => setProfileFilter(event.target.value)}><option value="">Everyone</option>{profiles.map(profile => <option value={profile.id} key={profile.id}>{profile.display_name}</option>)}</select></label><select aria-label="Filter content"><option>All types</option><option>Photo</option><option>Voice note</option><option>Document</option></select></div>}
function Library({ memories, allAssets, profiles, profileFilter, setProfileFilter, query, onCapture, onOpen, loading, supabase, context, spaces, flash }) { const selectedProfile = profiles.find(profile => profile.id === profileFilter); return <><PageHeading eyebrow="MEMORIES · FAMILY COLLECTION" title="Your family story." text={selectedProfile ? `Permitted memories linked to ${selectedProfile.display_name}.` : query?`Showing results for “${query}”.`:'Big milestones. Small moments. Everything worth keeping.'} action="＋ Add memory" onAction={onCapture}/><CollectionToolbar profiles={profiles} profileFilter={profileFilter} setProfileFilter={setProfileFilter}/>{loading?<LoadingContent/>:<><CuratedMemories supabase={supabase} context={context} spaces={spaces} assets={allAssets} query={query} flash={flash}/>{memories.length?<section className="memory-grid-new">{memories.map(memory=><MemoryCard key={memory.id} memory={memory} onOpen={onOpen}/>)}</section>:<EmptyCollection title="No source items found" text="Only items you can access are shown for this profile." onCapture={onCapture}/>}</>}</> }
...
function ProfileTagList({ tags = [] }) { return tags.length ? <div className="memory-profile-tags">{tags.map(tag => <span key={tag.id}>{tag.name}</span>)}</div> : null }
function MemoryCard({memory, onOpen}){return <button className="memory-card-new" onClick={() => onOpen(memory)}><div className={`card-art ${memory.voice?'voice':''}`}>{memory.voice?'〰〰〰':memory.type==='Photo'?'▧':'▤'}</div><div><h2>{memory.title}</h2><p>{memory.person} · {memory.date}</p><small>{memory.caption}</small><ProfileTagList tags={memory.profileTags}/></div></button>}
function Vault({ memories, profiles, profileFilter, setProfileFilter, onCapture, onOpen, loading }) { return <><PageHeading eyebrow="MY VAULT · ONLY YOU" title="Just for you." text="Your own space. Nothing shared unless you choose." action="＋ Add memory" onAction={onCapture}/><CollectionToolbar profiles={profiles} profileFilter={profileFilter} setProfileFilter={setProfileFilter}/>{loading?<LoadingContent/>:memories.length?<section className="memory-grid-new">{memories.map(memory=><MemoryCard key={memory.id} memory={memory} onOpen={onOpen}/>)}</section>:<EmptyCollection title="Your vault is clear" text="Private items you add will stay here unless you deliberately share them." onCapture={onCapture}/>}</> }
